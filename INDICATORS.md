...
| 예측처리 | 예측오차 최소화·느린 prior | 다음토큰예측, mood, **drive(G)** | [D5 eval_state](tools/eval_state.py) | 구현 |
| 주의도식(AST) | 자기 주의의 모델 | **attn_schema(C2)** | [D6 eval_schema](tools/eval_schema.py) | 구현/검증 |
| 행위성(agency) | 피드백 학습·유연한 목표추구 | **drive+proactive(G)** — 부분(외부, 미학습) | D5 + [tests/test_drive](tests/test_drive.py) | 부분 |
| 신체성(embodiment) | 출력-입력 수반성 모델 | time(부분/최소), G5 감각(경량) | — | 부분 |
...
- **런타임 층** (모델 밖, 학습 불필요): [`runtime/`](runtime/) —
  DriveState·StateManager·ProactiveEngine. drive는 mood/latent/workspace
  **내부 채널로만** 작용 (출력 우회 래퍼 금지). 정책은
  [`runtime/config.json`](runtime/config.json) (ModelConfig에 넣지 않음).
...
**proactive 출력·감정 서술은 지표 증거로 세지 않는다**(ELIZA). 행위성(G)은
drive→mood→출력 경로와 상태 복원·쿨다운 준수 같은 **말로 못 꾸며내는 구조**로만
부분 충족을 주장한다.
